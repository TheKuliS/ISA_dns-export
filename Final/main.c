...
*/
	while (1)
	{
		...
		char* domain_name = malloc(sizeof(char) * max_len);
		char* answer_data = malloc(sizeof(char) * max_len);
		uint16_t offset = 0;
		uint16_t rr_type;
		...
		if(ntohs(udp_header->source) == DNS_PORT)
		{
			...
			for (int i = 0; i < ntohs(dns_header->total_answer_RRs); i++) {
				...
				process_rr_data(dns_data, (offset + 10), rr_type, rr_data_length, &answer_data, 0, max_len);
